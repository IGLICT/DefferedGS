...
def build_scaling_rotation(s, r):
    L = jt.zeros((s.shape[0], 3, 3), dtype="float")
    R = build_rotation(r)
...
def create_rotation_matrix_from_direction_vector_batch(direction_vectors):
    ...
    direction_vectors = direction_vectors / jt.norm(direction_vectors, dim=-1, keepdim=True)
    # Create a batch of arbitrary vectors that are not collinear with the direction vectors
    v1 = jt.array([1.0, 0.0, 0.0], dtype="float32").expand(direction_vectors.shape[0], -1).clone()
    is_collinear = jt.all(jt.abs(direction_vectors - v1) < 1e-5, dim=-1)
    v1[is_collinear] = jt.array([0.0, 1.0, 0.0], dtype="float32")

    # Calculate the first orthogonal vectors
    v1 = jt.cross(direction_vectors, v1)
    v1 = v1 / (jt.norm(v1, dim=-1, keepdim=True))
    # Calculate the second orthogonal vectors by taking the cross product
    v2 = jt.cross(direction_vectors, v1)
    v2 = v2 / (jt.norm(v2, dim=-1, keepdim=True))
    # Create the batch of rotation matrices with the direction vectors as the last columns
    rotation_matrices = jt.stack((v1, v2, direction_vectors), dim=-1)
...
def colormap(img, cmap='jet'):
    ...
    img = jt.array(data / 255.).float().permute(2,0,1)
    plt.close()
    return img