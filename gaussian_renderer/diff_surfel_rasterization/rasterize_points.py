...
def RasterizeGaussiansCUDA(
    ...
    with jt.flag_scope(compile_options=proj_options):
        ...
        # print("render_num",rendered[0].item())
        # print("binningBuffer",geom_size)
        # print("binningBuffer",img_size)
        ...
        out_others = jt.array(jt.zeros([7,image_height,image_width],dtype='float32'))
        # print("binningBuffer",binningBuffer.shape)
        # print("render_num",rendered[0].item())
        # print("binningBuffer",binningBuffer.shape)
        # print("binningBuffer",imageBuffer.shape)
        binningBuffer,imageBuffer,out_color,out_others = jt.code(
            outputs=[binningBuffer,imageBuffer,out_color,out_others],
    ...
    # print("rasterize.py")
    # print(rendered.item())
    return rendered[0].item(), out_color, out_others, radii, geomBuffer, binningBuffer, imageBuffer
...
def RasterizeGaussiansBackwardCUDA(
    ...
    dL_dnormal = jt.zeros([P,3],dtype='float32')
    dL_dopacity = jt.zeros([P,1],dtype='float32')

    dL_dcov3D = jt.zeros([P,9],dtype='float32')
    # dL_dcov3D = jt.zeros([P,6],dtype='float32')

    dL_dsh = jt.zeros([P,M,3],dtype='float32')
    dL_dscales = jt.zeros([P,2],dtype='float32') 
    dL_drotations = jt.zeros([P,4],dtype='float32')
    # dL_dconic = jt.zeros([P,2,2],dtype='float32')
    # for i in [dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dnormal]:
    #     print(i.shape)
    # dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations,dL_dconic = jt.code(
    with jt.flag_scope(compile_options=proj_options):
        dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dnormal = jt.code(
            inputs=[background,means3D,radii,colors,scales,rotations,
                    cov3D_precomp,viewmatrix,projmatrix,dL_dout_color,sh,campos,geomBuffer,binningBuffer,imageBuffer,dL_dout_others],
            outputs=[dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dnormal],
            data={
                    'R':R,
                    'scale_modifier':scale_modifier,
                    'tan_fovx':tan_fovx,
